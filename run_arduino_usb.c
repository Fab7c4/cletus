...
static void *zsock_groundstation_sender = NULL;
static void *zsock_groundstation_receiver = NULL;
...
static void __attribute__((noreturn)) die(int code) {
    ...
    zdestroy(zsock_groundstation_sender, NULL);
    zdestroy(zsock_groundstation_receiver, NULL);
...
int main(int argc __attribute__((unused)),
   ...
   * many outdated messages. */
    ...
    if (NULL == zsock_log)
        die(1);
    zsock_groundstation_sender = setup_zmq_sender(GROUNDSTATION_SEND_CHAN, &zctx, ZMQ_PUB, 10, 512);
    if (NULL == zsock_groundstation_sender)
        die(1);
    zsock_groundstation_receiver = setup_zmq_receiver(GROUNDSTATION_RECV_CHAN, &zctx, ZMQ_SUB, NULL, 1, 500);
    if (NULL == zsock_groundstation_receiver)
        die(1);
 ...
 */
    ...
    zmq_pollitem_t poll_groundstation = {
        /* Inputs -- in this case, our incoming actuator commands. */
            .socket = zsock_groundstation_receiver,
            .fd = -1,
            .events = ZMQ_POLLIN,
            .revents = 0
    };
   ...
   * mostly out of laziness. */
    ...
    while (1)
    {
        ...
        int zs = zmq_send(zsock_groundstation_sender, zmq_buffer, packed_length, 0);
        if (zs < 0) {
            txfails++;
        } else {
        ...
        if (poll_groundstation.revents & ZMQ_POLLIN) {
            ...
            const int zr = zmq_recvm(zsock_groundstation_receiver, zmq_buffer,
            ...
            if (actuators != NULL){
                ...
                arduino_send_message((uint8_t*) &output, sizeof(output));
            }
